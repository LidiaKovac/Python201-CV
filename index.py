...
img = cv.imread('./images/1.png') #0 means B/W
img2 = cv.imread('./images/2.png')

sum = img + img2
#cv.imshow("sum", sum)
# cv.imshow("sum", img2 - img) #you can do whatever calculation you want here
noise = random.randint(0, img.shape[0]) * 10
#randint generates gaussian noise.
...
#cv.waitKey(0) #finished program when pressing 0
...
for i,col in enumerate(color):
    ...
    plt.xlim([0,256])
plt.show() #very useful in photography
#how would I save this as an image?
